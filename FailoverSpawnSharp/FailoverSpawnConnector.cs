...
namespace FailoverSpawnSharp
{
    public class FailoverSpawnConnector
    {
        ...
        public FailoverSpawnConnectionInfo Connect()
        {
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(_connectionString))
                {
                    ...
                    if(connection.State != ConnectionState.Open)
                        throw new Exception("Connection state was not open.");
            ...
            {
                Console.WriteLine($"[{DateTime.Now}] Exception occurred: {exception}");
                connectionInfo = new FailoverSpawnConnectionInfo()
                                 {
                                     ConnectionDateTime = DateTime.Now.ToString(CultureInfo.InvariantCulture)
                                 };
            }
